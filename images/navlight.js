...
$.fn.topSuction = function(option) {
	...
	function fix(){
		...
		if ( fTop < dTop && feBottom > dTop) {
			...
			$self.children(navbox).width( $self.width() );
			if (fixedFunc) {
...
$.fn.navLight = function(option, callback) {
	...
	var open_datanum = option.open_datanum;		
	...
	if(open_toptext){
		...
		if( $navarea.attr('data-toptext') !== 'on' ){
			...
			$navarea.find('ul').prepend( cateone );
		}
	};
	...
	if (open_datanum) {
		...
		if( datanum > ' ' ){
			$navs.find('li').css('float','left');
			$navs.find('li').css('width',  ($navs.width() ) / datanum)
		};
	};
